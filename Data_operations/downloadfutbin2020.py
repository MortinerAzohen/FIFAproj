...
C = open('FutBinCards20.csv', 'w')
C.write(','.join(cardColumns) + '\n')
D = open('FutBinDetailed20.csv', 'w')
...
for key, value in fifa.items():
    ...
    for page in range(1, int(TotalPages) + 1):
        ...
        print(df)
        df.to_csv('FutBinCards20.csv', mode='a', header=False, sep=',', encoding='utf-8', index=False)
        pd.read_json(json_data).transpose().to_csv('FutBinDetailed20.csv', mode='a', header=False, sep=',', encoding='utf-8', index=True)